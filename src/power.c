/*++
	...
	Abstract:
		...
		Contains ST power-on and power-off functionality
...
)
{
	...
	ControllerContext = (FTS_CONTROLLER_CONTEXT*)devContext->TouchContext;
	...
	if (IsEqualGUID(&GUID_ACDC_POWER_SOURCE, SettingGuid))
	{
		...
		case PoAc:
			...
			status = FtsChangeChargerConnectedState(
				ControllerContext,
				SpbContext,
				0
			);
	...
	else if (IsEqualGUID(&GUID_CONSOLE_DISPLAY_STATE, SettingGuid))
	{
		...
		case 0:
			...
			{
				status = FtsSetReportingFlags(
					ControllerContext,
					SpbContext,
					REPORTING_WAKEUP_GESTURE_MODE,
					NULL
				);
...
--*/
{
	...
	//
	status = FtsChangeSleepState(
		controller,
		SpbContext,
		DEVICE_CONTROL_SLEEP_MODE_OPERATING);
...
--*/
{
	...
	status = FtsChangeSleepState(
		ControllerContext,
		SpbContext,
		DEVICE_CONTROL_SLEEP_MODE_SLEEPING);
